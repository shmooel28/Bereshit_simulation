...
public class Bereshit_101{
	...
	public static void main(String[] args) {
		...
		System.out.println("time,	vs,	hs,	dist, alt, ang,	weight,	acc,	full");
		double NN = 0.7; // rate[0,1]
		// ***** main simulation loop ******

		double p =0.04, i = 0.0003, d = 0.2, max_i = 0.5;


		PID pid =new PID(p,i,d,max_i);
		PID pid2 =new PID(p,i,d,max_i);
		...
		while(alt>0) {
			...
			if(time%10==0 || alt<10000) {
				System.out.println(time+"	"+vs+"	"+hs+"	"
						+dist+"	"+alt+"	"+ang+"	"+weight+"	"+acc+"	"+fuel) ;
			}
			// over 2 km above the ground
			if(alt>4500) {    // maintain a vertical speed of [20-25] m/s// g
				NN += pid.update((vs - 31), dt);

			}
			else if (alt>1500){
				NN += pid.update((vs - 23), dt);
			}
			else if (alt>750){
				NN += pid.update((vs - 18), dt);
			}
			else if (alt>100){
				NN += pid2.update((vs - 12), dt);
//
			}
			else if (alt>50){
				NN += pid2.update((vs - 8), dt);
			}
			else if (alt>20){
				NN += pid2.update((vs - 4), dt);
			}
			else {
				NN+=pid2.update((vs - 2), dt);
			}
...
//		System.out.println("time: "+ time+",vs: "+vs+",hs: "+hs+",dist: "
//				+dist+",alt: "+alt+",ang: "+ang+",we: "+weight+",acc: "+acc+" ,fuel: "+fuel) ;
	}
}
